<img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=FDB2DA&height=80&section=header"/>

<div align="center">
    <a href="https://git.io/typing-svg">
        <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=45%&duration=3000&pause=200&color=FDB2DA&center=true&vCenter=true&width=800&height=50%&lines=Hello+there!;It's+me,+techmoocher.;Welcome+to+my+profile!" alt="Typing SVG" />
    </a>
</div>
<br>

## ABOUT ME 🦊

<img align="right" alt="A quick brown fox jumps over a lazy dog" src="./images/a-quick-brown-fox.gif" width=25% height=35%/>
<ul>
    <li>👨‍💻&nbsp; <b>Name:</b> Phuc (Benjamin) Nguyen (a.k.a <i><strong style="color:magenta;">techmoocher</strong></i>).</li><br>
    <li>🪪&nbsp; <b>Title: </b> High school senior @ VNUHCM High School for the Gifted.</li><br>
    <li>💖&nbsp; <b>Hobbies: </b> Running 🏃‍♂️, Cycling 🚴‍♂️, Piano 🎹, Reading 📖.</li>
</ul>

### Me doing my hobbies :)
<div>
    <img alt="Tom running (Tom&Jerry)" src="./images/running.gif" width="100px"/>
    <figcaption>Running</figcaption>
    <img alt="Riding bicycle" src="./images/cycling.gif" width="200px"/>
    <figcaption>Cycling</figcaption>
    <img alt="Piano playing" src="./images/piano.gif" width="200px"/>
    <figcaption>Piano</figcaption>
    <img alt="Snoopy reading and laughing" src="./images/reading.gif" width="200px"/>
    <figcaption>Reading</figcaption>
</div>

<img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=FDB2DA&height=80&section=footer"/>
